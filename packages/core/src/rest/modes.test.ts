...
import { createRestTransport } from './index.js'
...
async function client(transport: SyncTransport): Promise<PhotonClient> {
 ...
 */
function restTransport(verdict: 'accept' | 'reject' = 'accept'): SyncTransport {
...
const modes: [string, (verdict?: 'accept' | 'reject') => SyncTransport][] = [
  ['engine-native', engineTransport],
  ['rest-backed', restTransport],
]

describe.each(modes)('collection mode: %s', (_name, transport) => {
  it('reflects the write optimistically and then confirms it', async () => {
    const photon = await client(transport())
  ...
  it('rolls the record back when the server refuses', async () => {
    const photon = await client(transport('reject'))
    const handle = photon.upsert('issues', 'r1', { id: 'r1', title: 'a' })
    await handle.local
