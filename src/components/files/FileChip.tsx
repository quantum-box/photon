...
export function FileChip({ file, onPreview, onRemove }: FileChipProps) {
  ...
  return (
    <div
      className="inline-flex items-center gap-2 rounded-lg px-3 py-2 cursor-pointer transition-colors group/chip"
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-xs font-medium truncate" style={{ color: 'var(--text-primary)', maxWidth: 160 }}>
          {file.name}
        </p>
      ...
      {onRemove && (
        <button
          ...
          className="w-5 h-5 rounded flex items-center justify-center text-xs opacity-0 group-hover/chip:opacity-100 transition-opacity cursor-pointer"
